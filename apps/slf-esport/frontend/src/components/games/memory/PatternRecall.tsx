  ...
  return (
    <div className="h-full flex flex-col">
        ...
        <div className="mt-4 p-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
          ...
          <div className="flex justify-center gap-2 flex-wrap">
            {colors.map((color) => (
              <button
                key={color}
                className="w-12 h-12 sm:w-14 sm:h-14 rounded-lg shadow-md hover:scale-110 active:scale-95 transition-transform touch-manipulation"
                style={{ backgroundColor: color }}
                onClick={() => {
                  // Find first non-active cell or last clicked
      ...
      <div className="mt-4 flex gap-2">
        {phase === GamePhase.RECALL && (
          <Button variant="primary" fullWidth onClick={submitPattern}>
            ✓ Valider
          </Button>
        )}
        {phase === GamePhase.COMPLETED && (
          <Button variant="outline" fullWidth onClick={startGame}>
            🔄 Recommencer
          </Button>
        )}
        {phase === GamePhase.READY && (
