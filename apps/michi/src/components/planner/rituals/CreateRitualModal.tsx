/**
 * CreateRitualModal Component - Modal for creating a new ritual
...
import { useCreateRitual } from '@/hooks/api/useRituals'
import type { RitualCategory, RitualTask } from '@/types/api'
...
const CATEGORY_OPTIONS: Array<{ value: RitualCategory; label: string; emoji: string }> = [
  { value: 'morning', label: 'Matin', emoji: '🌅' },
  { value: 'evening', label: 'Soir', emoji: '🌙' },
  { value: 'daily', label: 'Quotidien', emoji: '📅' },
...
export function CreateRitualModal({ isOpen, onClose }: CreateRitualModalProps) {
  const [label, setLabel] = useState('')
  const [icon, setIcon] = useState('✅')
  const [category, setCategory] = useState<RitualCategory>('custom')
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="➕ Créer un nouveau rituel" size="md">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              disabled={!currentTask.trim() || createRitual.isPending}
            >
              ➕
            </Button>
          </div>
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
          ...
          <Button
            ...
            variant="primary"
            size="md"
            disabled={!label.trim() || createRitual.isPending}
          >
            {createRitual.isPending ? (
              <>
        ...
        {/* Error display */}
        {createRitual.isError && (
          <div className="p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
            <p className="text-sm text-red-600 dark:text-red-400">
              ❌ Une erreur est survenue lors de la création du rituel. Veuillez réessayer.
            </p>
          </div>
        )}
      </form>
