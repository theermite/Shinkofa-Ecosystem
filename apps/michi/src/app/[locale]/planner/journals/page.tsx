...
export default function JournalsPage() {
  ...
  useEffect(() => {
    ...
    import('@ermite-widgets/daily-journal/src/store').then(({ configureApiSync, loadFromApi, useJournalStore }) => {
      ...
      console.log(`✅ Daily Journal API sync configured for user: ${currentUserId}, token: ${token ? 'present' : 'MISSING'}`)
      ...
      if (loadFromApi && token) {
        loadFromApi().then((apiData) => {
          ...
          if (apiData && apiData.lastUpdated) {
            console.log('✅ Daily Journal data loaded from API:', apiData.lastUpdated)
            ...
            console.log(`📊 Comparing: API=${apiTimestamp} (${apiJournalCount} journals) vs Local=${localTimestamp} (${localJournalCount} journals)`)
            ...
            console.log('ℹ️ No Daily Journal data on server')
